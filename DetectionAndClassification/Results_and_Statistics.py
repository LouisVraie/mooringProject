...
image_directory = 'c:/Users/lotus/Desktop/Mapping Lerins/Mapping Lerins/2023 09 ZONE 2/'
image_paths = [os.path.join(image_directory, file) for file in os.listdir(image_directory) if file.lower().endswith(('.jpg', '.jpeg', '.png'))]

model_path = 'C:/Users/lotus/Desktop/Mooring_Project/code/runs/detect/train8/weights/last.pt'
...
def Morphology(x_min, y_min, x_max, y_max, img):
    ...
    opening = cv2.morphologyEx(gray_roi, cv2.MORPH_OPEN, kernel)

    # morphological operations 
    dilated_roi = cv2.dilate(opening, kernel, iterations=1)
    eroded_roi = cv2.erode(dilated_roi, kernel, iterations=1)

    # Threshold the HSV image to get only blue colors
    mask = cv2.inRange(eroded_roi, 170, 255)

    # Bitwise-AND mask and original image
    res = cv2.bitwise_and(eroded_roi,eroded_roi, mask= mask)

    # Display the resulting image
    # cv2.imshow('edges_img', res)
    # cv2.waitKey(0)
