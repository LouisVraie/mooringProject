...
CONSTANTS_FILE = os.path.join(SCRIPT_DIR, 'contants.yaml')

with open(CONSTANTS_FILE, 'r') as stream:
    constants = yaml.safe_load(stream)
    constants['inp_img_pth'] = os.path.join(SCRIPT_DIR, constants['inp_img_pth'])
    constants['inp_lab_pth'] = os.path.join(SCRIPT_DIR, constants['inp_lab_pth'])
    constants['out_img_pth'] = os.path.join(SCRIPT_DIR, constants['out_img_pth'])
    constants['out_lab_pth'] = os.path.join(SCRIPT_DIR, constants['out_lab_pth'])
    constants['out_img_w_bb_pth'] = os.path.join(SCRIPT_DIR, constants['out_img_w_bb_pth'])
    CONSTANTS = constants
...
def get_album_bb_list(yolo_bbox, class_names):
    ...
    bbox_values = [min(max(float(value), 0), 1) for value in str_bbox_list[1:]]
...
def draw_yolo(image, labels, file_name, img_extension: str = "png"):
    """
    ...
    Args:
        ...
        img_extension (str, optional): The extension of the new image. Defaults to png.
    """
    H, W = image.shape[:2]
    for label in labels:
    ...
    cv2.imwrite(f"{CONSTANTS['out_img_w_bb_pth']}/{file_name}.{img_extension}", image)
    # cv2.imshow(f"{file_name}.{img_extension}", image)
    # cv2.waitKey(0)
...
def get_augmented_results(image, bboxes, transforms):
    """
    ...
    - transforms (list): List of TransformsSeqType to apply on images
    ...
    transform = A.Compose(transforms, bbox_params=A.BboxParams(format='yolo'))
...
def save_augmentation(trans_image, trans_bboxes, trans_file_name, save_background, is_drawn, img_extension: str = "png"):
    """
    ...
    Parameters:
        ...
        save_background (bool, optional): Save images with background only. Defaults to True.
        is_drawn (bool, optional): Save extra images with YOLO bounding boxes. Defaults to False.
        img_extension (str, optional): The extension of the new image. Defaults to png.
    Returns:
        None
    """
    ...
    if tot_objs:
        ...
        if not has_negative_element(trans_bboxes):
            ...
            save_aug_image(trans_image, CONSTANTS["out_img_pth"], trans_file_name + "." + img_extension)
            
            if is_drawn:
                # Draw bounding boxes on the augmented image
                draw_yolo(trans_image, trans_bboxes, trans_file_name, img_extension)
        else:
...
def get_updated_transforms(transforms: list, image, img_max_size):
    if img_max_size is not None:
        image_width = min(image.shape[0], image.shape[1], img_max_size)
        image_height = image_width
    else:
        image_width = image.shape[1]
        image_height = image.shape[0]

    update_transforms = transforms.copy()
    
    # Add CenterCrop to transforms
    update_transforms.insert(0, A.CenterCrop(width=image_width, height=image_height))
...
def get_image_data():
    ...
    for img_file in imgs:
        print(f"Reading data of {img_file}\n")
        image_data.append(get_inp_data(img_file))

    return image_data
        
def run_yolo_augmentor(image_data, transforms: list, img_max_size: int, img_suffix: str="", save_background: bool=True, is_drawn: bool=False, img_extension: str = "png"):
    """
    ...
    Args:
        transforms (list): List of TransformsSeqType to apply on images
        img_max_size (int): Max size of an image to crop
        img_suffix (str, optional): Image suffix. Defaults to "".
        save_background (bool, optional): Save images with background only. Defaults to True.
        is_drawn (bool, optional): Save extra images with YOLO bounding boxes. Defaults to False.
        img_extension (str, optional): The extension of the new image. Defaults to png.
    """
    
    image, gt_bboxes, aug_file_name = image_data
    
    # Edit image name
    aug_file_name = aug_file_name + img_suffix
    print(f"Processing {aug_file_name}\n")
    
    # Update transform to update specific image data
    update_transforms = get_updated_transforms(transforms, image, img_max_size)
    
    # Get the augmented result
    aug_img, aug_label = get_augmented_results(image, gt_bboxes, update_transforms)
    
    if len(aug_img) and (len(aug_label) or save_background):
        save_augmentation(aug_img, aug_label, aug_file_name, save_background, is_drawn, img_extension=img_extension)